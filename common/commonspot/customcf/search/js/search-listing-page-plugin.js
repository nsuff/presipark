    ...
    var SearchListing = function(element, options)
    {
		...
		this.updateSearchResultsResponse = function(rspContent)
		{
			...
			if(isNaN(numResults) || numResults == 0){	
				...
				jQuery(".results .result-count",element).html("<h4>Showing results "+startNum+"-"+endNum+" of "+numResults+"</h4>");
		...
		this.showSearchResultsLI = function(item)
		{
			...
			for (var i = 0; i < metaFieldArr.length; i++) {			
				...
				if(key != "Image_URL" && key != "Image_Alt_Text" && key != "Sites" && key != "Abstract" && key != "Title"  && key != "PageURL" && key != "csPageID" && key in meta){				
					metaHTML += '<li><span class="ListingMeta-label">'+key.replace(/_/g, ' ')+':</span> '+meta[key]+'</li>';
				}
			}
			if(metaHTML.length > 0) metaHTML = '<div class="col-sm-12 col-md-4 col-md-push-8"><div class="ListingMeta"><ul>' + metaHTML + '</ul></div></div>';
			...
			var sitesHTML = "";
			if("Sites" in meta && meta["Sites"].length && meta["Sites"].split(",").length == 1){
			...
			if(metaHTML.length > 0){
				itemHTML += metaHTML + '<div class="col-sm-12 col-md-8 col-md-pull-4">';
			}else{
				itemHTML += '<div class="col-sm-12 col-md-12">'; //no meta, make it full width
			}
